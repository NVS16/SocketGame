...
io.on('connection', function (socket) {
    ...
    // room socket
    socket.on('joinroom', function(data) { console.log(data);
            
            if(gameController[data.roomname]["connected-clients"] < 2) {
                socket.join(data.roomname);
                socket.roomName = data.roomname ;
                socket.memberDetails = {"name" : data.name , 
                                        "id" : socket.id };
    ...
    socket.on('engage', function(e) {
      ...
      socket.on("draw-first" , function(msg){
        console.log(msg);
        socket.broadcast.to(socket.roomName).emit('see-drawing', 'Your partner will draw first');
    });
